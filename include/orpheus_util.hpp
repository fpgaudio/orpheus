...
inline auto bitSet(int16_t value, std::size_t bit) -> bool {
  return static_cast<uint16_t>(value) ==
         (static_cast<uint16_t>(value) | (1U << bit));
}

/**
 * @brief Calculates the sine of the given angle.
 *
 * @param angleRad The fraction of a full circle. A value of INT16_MAX means
 * 2PI.
 *
 * @return 16bit fixed point sine value (4.12)
 */
template <typename T> inline auto sin(T angleRad) -> int16_t {
  return sin(
      static_cast<int16_t>(angleRad % std::numeric_limits<int16_t>::max()));
}

template <> inline auto sin(int16_t angleRad) -> int16_t {
  ...
  // Calculate whether we should carry. If the value is greater than pi/2, we
  // will carry.
  const auto carry = angleRad < 0;

  // If the 13th bit is set, that means that this is a value greater than 2^13,
  // implying it is negative. Let us correct that. and invert the bitmask.
  ...
  // Select only the 2^15 bits, shift back down by one spot to correct for the
  // shift at the start.
  constexpr auto allBits15U = 0x7FFFU;
  angleRad = static_cast<int16_t>(
      (static_cast<uint16_t>(angleRad) & allBits15U) >> 1U);
  ...
  const auto outCast =
      carry ? (-static_cast<int16_t>(out)) : static_cast<int16_t>(out);
